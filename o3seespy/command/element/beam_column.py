...
class ElasticBeamColumn(ElementBase):
    ...
    def __init__(self, osi, ele_nodes, big_a, big_e, iz, transf, mass=None, '-c_mass', big_g, big_j, iy):
        ...
        osi.n_ele += 1
        self._tag = osi.n_ele
        self._parameters = [self.op_type, self._tag, *self.ele_nodes, self.big_a, self.big_e, self.iz, self.transf.tag, self.'-c_mass', self.big_g, self.big_j, self.iy]
...
class ModElasticBeam2dmass(ElementBase):
    ...
    def __init__(self, osi, ele_nodes, big_a, big_e, iz, k11, k33, k44, transf, mass_dens, '-c_mass'):
        self.ele_nodes = ele_nodes
        self.big_a = float(big_a)
        self.big_e = float(big_e)
        self.iz = float(iz)
        ...
        osi.n_ele += 1
        self._tag = osi.n_ele
        self._parameters = [self.op_type, self._tag, *self.ele_nodes, self.big_a, self.big_e, self.iz, self.k11, self.k33, self.k44, self.transf.tag, '-mass', self.mass_dens, self.'-c_mass']
        self.to_process(osi)


class ElasticTimoshenkoBeammass(ElementBase):

    def __init__(self, osi, ele_nodes, big_e, big_g, big_a, iz, avy, transf, mass_dens, '-c_mass', jx, iy, iz_2, avz, mass=None, '-c_mass):
        self.ele_nodes = ele_nodes
        self.big_e = float(big_e)
        self.big_g = float(big_g)
        self.big_a = float(big_a)
        ...
        osi.n_ele += 1
        self._tag = osi.n_ele
        self._parameters = [self.op_type, self._tag, *self.ele_nodes, self.big_e, self.big_g, self.big_a, self.iz, self.avy, self.transf.tag, '-mass', self.mass_dens, self.'-c_mass', self.jx, self.iy, self.iz_2, self.avz, self.'-c_mass]
        if getattr(self, 'mass') is not None:
            self._parameters += ['-mass', self.mass]
        self.to_process(osi)
