...
def create():
    df = pd.read_csv('nd-material-ffps.csv')
    od = {'obj': [],
          'o3_name': [],
          'cpp_name': []}
    for i in range(len(df)):
        ...
        if ffp == '-':
            continue
        obj = df['obj'].iloc[i]
        lines = open(up.OPENSEES_SOURCE_PATH + ffp).read().splitlines()
        on = 0
        if not df['mat'].iloc[i]:
            objname = obj
        else:
            objname = f'{obj}Material'
        params = []
        for line in lines:
    ...
    df.to_csv('nd-material-set-parameters.csv', index=False)
