...
class PressureIndependMultiYield(NDMaterialBase):
    ...
    def __init__(self, osi, nd, rho, ref_shear_modul, ref_bulk_modul, cohesi, peak_shear_stra, friction_ang=0., ref_press=100., press_depend_coe=0., no_yield_surf=20, yield_surf=None):
        self.nd = float(nd)
        self.rho = float(rho)
        ...
        osi.n_mat += 1
        self._tag = osi.n_mat
        self._parameters = [self.op_type, self._tag, self.nd, self.rho, self.ref_shear_modul, self.ref_bulk_modul, self.cohesi, self.peak_shear_stra, self.friction_ang, self.ref_press, self.press_depend_coe, self.no_yield_surf]
        if yield_surf is not None:
